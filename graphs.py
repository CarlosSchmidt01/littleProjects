...
def roadsAndLibraries(n, c_lib, c_road, cities):
    ...
    if c_lib < c_road:
        ...
        def dfs(i, cluster):
            if not visited[i]:
                nodes_per_cluster[cluster] = (nodes_per_cluster.get(cluster, 0) + 1)
            visited[i] = True
            my_neighbours = []
            try:
        ...
        for road in cities:
            neighbours[road[0]] = (neighbours.get(road[0], []) + [road[1]])
            neighbours[road[1]] = (neighbours.get(road[1], []) + [road[0]])
